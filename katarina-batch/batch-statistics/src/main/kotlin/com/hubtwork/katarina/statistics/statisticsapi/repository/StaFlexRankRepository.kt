...
interface StaFlexRankRepository: JpaRepository<StaFlexRank, Long> {
    ...
    @Query("select * from STA_flex_match where summoner_name = :summoner_name and season = :season and champion_id = :champion_id and lane = :lane", nativeQuery = true)
    ...
    @Query("update STA_flex_match set kill_count = kill_count + :kill, death_count = death_count + :death, assist_count = assist_count + :assist, game_win_count = game_win_count + :win, game_all_count = game_all_count + 1 where summoner_name = :summoner_name and season = :season and champion_id = :champion_id and lane = :lane",nativeQuery = true)
