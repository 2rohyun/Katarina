...
interface StaEventMatchRepository: JpaRepository<StaEventMatch, Long> {
    @Query("select distinct champion_id from STA_event_match where summoner_name = :summoner_name and season = :season", nativeQuery = true)
    ...
    @Query("select * from STA_event_match where champion_id = :champion_id",nativeQuery = true)
    fun getAllByChampionId(@Param("champion_id")champion_id: Int): MutableList<StaEventMatch>
}